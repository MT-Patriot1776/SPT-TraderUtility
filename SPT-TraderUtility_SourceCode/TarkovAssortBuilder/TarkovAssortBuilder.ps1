...
# --- Bootstrap: prefer PowerShell 7; fall back to Windows PowerShell STA ---
...
'@
    ...
    # Price and description maps
    Update-Log $LogTextBox "Computing trader prices..."
    $priceMap = @{}
    $descMap  = @{}
    foreach ($hid in $ids) { $descMap[$hid] = "Place Comment HERE" }

    foreach ($it in $items) {
        $desc = $it.description; if (-not $desc) { $desc = "Place Comment HERE" }
        $descMap[$it.id] = $desc
...
foreach ($id in $ids) {
    ...
    if ($CompactJson) { $jsonOut = Minify-Json $jsonOut }
